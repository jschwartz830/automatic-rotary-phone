...
-- Timesheets are never hard-deleted by the app -- a parent/co-admin "deletes"
-- one they generated by accident by setting deleted_at via timesheets_update
-- above (and can restore it the same way), any time before it's been marked
-- paid or locked. No delete policy is granted, so the row can't be removed
-- outright even via a direct API call.
...
-- Payment records are never hard-deleted by the app -- deleting a timesheet
-- soft-deletes its payment record too (deleted_at, via payment_records_update_manager
-- above), so payment history is never actually destroyed. No delete policy is
-- granted.
